...
\chapter{Revisão de Literatura}\label{sec:revisao}
    ...
    \toprule
        ...
        \textit{sleep quality} \textbf{AND} \textit{functioning} \textbf{AND} \textit{bipolar disorder} & 135 & 28 & 17 & 9 \\ \midrule
        ...
        (\textit{hypersomnia} \textbf{OR} \textit{insomnia}) \textbf{AND} (\textit{relapse} \textbf{OR} \textit{recurrence}) \textbf{AND} (\textit{bipolar disorder} \textbf{OR} \textit{major depressive disorder}) & 280 & 15 & 9 & 5 \\ \bottomrule
    \end{tabular}%
    }
    \fonte{Próprio Autor}
    \end{table}
    ...
    dessa forma, foram incluídos mais 3 artigos nesta revisão de literatura.
    ...
    Para avaliação dos parâmetros do sono, a maioria dos estudos selecionados
    se utilizou do instrumento \textit{Pittsburgh Sleep Quality Index} (PSQI),
    que é detalhado posteriormente na \autoref{sec:psqi}
    ...
    com TB, depressivos, pacientes subsindrômicos ou com características ciclotímicas,
    também apresentam alterações nos padrões de sono piores em relação aos grupos
    controle.
...
Sono reduzido foi estatisticamente diferente entre o grupo dos bipolares e cada um dos dois grupos de TDM. Estudo também aponta que sintomas individuais podem ser úteis na diferenciação do TB para o TDM &
 \\ \midrule
    ...
    Limitação: não houve equivalência em relação a uso de medicação nos grupos \\ \bottomrule
\end{longtabu}
    \end{landscape}
